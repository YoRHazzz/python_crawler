...
class NovelDownloader:
    ...
    def get_novel_dict(self, chapter_url_list: list) -> dict:
        ...
        print(" 重试失败章节 ".center(60, '*'))
        result.retry_failed_urls()
        result.show_urls_status()

        print(" 分离章节内容 ".center(60, '*'))
        process_number = int(self.downloader.config.ini['multi']['process_number'])
        ...
        for i in range(process_number):
            Process(target=self.fill_novel_dict, args=(chapter_url_list[i::process_number]
                                                       , result.urls_detail, queue)).start()
        for i in tqdm(range(len(result.finished_urls)), total=len(result.finished_urls),
                      desc="分离章节内容", unit="章节", postfix={"process": process_number}):
            queue.get()

        return result.urls_detail
